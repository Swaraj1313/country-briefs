import { WB_BASE, ACTUALS_START, ACTUALS_END, FORECAST_FROM, PROJECTION_END } from "./config";

const IMF_PROXY = "https://imf-backend-xir1.onrender.com";

export async function fetchIMFSeries(imfCode, indicatorCode, includeProjections = false) {
  try {
    const url = `${IMF_PROXY}/api/imf/${indicatorCode}/${imfCode}`;
    ...
    return Object.entries(raw)
      ...
      .map(d => ({ year: d.year, value: parseFloat(parseFloat(d.value).toFixed(2)), isForecast: d.year >= FORECAST_FROM }))
...
export async function fetchWBSeries(wbCode, indicatorCode) {
  try {
    ...
    return (json[1] ?? [])
      ...
      .map(d => ({ year: parseInt(d.date), value: parseFloat(parseFloat(d.value).toFixed(2)), isForecast: false }))
...
export async function fetchWBSnapshot(wbCode, indicatorCode) {
  try {
    ...
    return { value: parseFloat(parseFloat(hit.value).toFixed(2)), year: parseInt(hit.date) };
  } catch { return { value: null, year: null }; }
}
